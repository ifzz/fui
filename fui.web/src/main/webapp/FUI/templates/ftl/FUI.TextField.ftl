<#--
 ...
 # 系统名称: FUI
 # 模块名称: JRES
 # 文件名称: FUI.textField.ftl 
 # 作者：hanyin
 # 邮箱：hanyin@hundsun.com
 # 软件版权: 恒生电子股份有限公司
 # 功能描述： Form表单不含有隐藏域的输入框组件FUI.TextField的DOM结构定义
 # 修改记录:
 # 修改日期         修改人员         修改说明
 # 20120807		hanyin	 创建
...
<@validateAndSet name="id" default=genId("f-textField") />
...
<#assign _class>${boolValue(disabled,false)?string("f-state-disabled ","")}${boolValue(readonly,false)?string("f-state-readonly","")} ${class}</#assign>
...
<#if _hasIcon&&!_isInnerIcon>
	...
	<div id="${id}-wrapper" class="f-textField-wrapper">
		<input id="${id}" type="${type}" class="f-textField ${class}" tabIndex="${tabIndex}" name="${name}" check="${check}" <#t>
		...
		style="width:${_width}px;height:${_height}px;padding:0px;" <#t>
		errorMsgTarget="parent" <#t>
		value="${value}">
...
<@script>
$(function() {
	$("#${id}").FTextField({hasHiddenField: false});
});
</@script>
